...
FUNCTION frm_Edit( Self )
   ...
   FOR EACH aItem IN ::aControlList
      ...
      IF ::nEditStyle == 1 .OR. ::nEditStyle == 2
         nLen := Len( aItem[ CFG_CAPTION ] ) + aItem[ CFG_FLEN ] + 4 + iif( Empty( aItem[ CFG_VTABLE ] ), 0, aItem[ CFG_VLEN ] + 4 )
      ELSE
         nLen := Max( aItem[ CFG_FLEN ] + iif( Empty( aItem[ CFG_VTABLE ] ), 0, aItem[ CFG_VLEN ] + 4 ), Len( aItem[ CFG_CAPTION ] ) )
      ENDIF
      ...
      IF ! Empty( aItem[ CFG_VTABLE ] )
         gui_CreateLabel( iif( ::lWithTab, oTab, ::oDlg ), @aItem[ CFG_VCONTROL ], ;
            nRow2, nCol2 + ( ( aItem[ CFG_FLEN ] + 4 ) * 12 ), nLen * 12, ;
